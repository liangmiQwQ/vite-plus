...
|-----------|---------|--------|
| Zero external dependencies (raw FFI) | ~100KB (vs `windows` crate) | Done |
| No `core::fmt` (avoid `eprintln!`/`format!`/`.unwrap()`) | ~100KB | Done |
| Workspace profile: `lto="fat"`, `codegen-units=1`, `strip="symbols"`, `panic="abort"` | Inherited | Done |

**Binary size**: ~100KB on Windows (down from ~207KB with `windows` crate, ~223KB with `windows` + `core::fmt`).
...
The vite-plus trampoline is significantly simpler because it doesn't need to embed data in PE resources — it just reads its own filename, finds `vp.exe` at a fixed relative path, and spawns it. The ~60KB size difference from uv-trampoline comes from using standard `fn main()` and `std::process::Command` instead of nightly-only `#![no_main]` and raw `CreateProcessW`.
