...
async fn setup_vp_wrapper(bin_dir: &vite_path::AbsolutePath, refresh: bool) -> Result<(), Error> {
    ...
    #[cfg(windows)]
    {
        ...
        if should_create {
            ...
            // Best-effort cleanup of all accumulated .old files from previous refreshes
...
pub(crate) async fn cleanup_legacy_windows_shim(bin_dir: &vite_path::AbsolutePath, tool: &str) {
    ...
    // Only remove if it's a text file (not a symlink or binary)
    let sh_path = bin_dir.join(tool);
    if let Ok(content) = tokio::fs::read(&sh_path).await {
